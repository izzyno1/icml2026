...
官方 API 交互登录和受保护下载已实际成功：3 个完整 PDF 响应，第四个中断。
当前拥有 5 篇文献、7 个 PDF，其中 2 篇 ICML 2026 主轨目标；真实论文闭环 0。
首篇原始投稿/当前附件分别为 49/65 页，当前字节与用户先前保存件一致。
PyMuPDF 1.28.2 在唯一 .venv；新增 API 流程无新依赖、凭据文件或权限调整。

最新完整验收 **150 项：148 通过、2 跳过、0 失败**，含四项实际强制进程中断恢复。
两个跳过是已有 Windows 符号链接权限案例。活跃代码哈希
`4380e12472879198cd06918fa6bac662ce0767633911f14aa5df2b3538cb4b65`，规则哈希
`600097d4e4c9e7ab198777e97e61251761ea35acf7ae5eeb50c6c2655ad9fbc4`。
原 AGENTS.md 未改；本会话继承的 full-access/no-approval 不是重新验收的原生边界。

两轮真实 Pro 历史返回与限定事实核对已保存；代码变化后不替换旧绑定。新短任务
已复核字节和新增来源身份，146 页完整文本包已准备进行新一轮真实审读。状态以
P2_CHECKPOINT.md、published/status.json 和本地账本为准。人工审计未做。

下载器每读 1 MiB 前后仍检查预算/时间/大小；精确作者 URL 逐条放行，角色限前作/
预印本。第四份旧下载的具体中断原因未知，修正后尚未重新实测该文件。详见
OPENREVIEW_API_LOGIN.md。以下旧测试数量和访问失败记录均为历史。
