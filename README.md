...
初筛实际返回 10 项主张；另一个 Pro 对话实际返回 6 项复核主张，均已隔离导入，
本地分别完成限定范围的原文核对与两轮对照。账号记忆隔离无法验证，人工审计未做。
另两篇主轨候选的单次受保护下载均为 HTTP 403。
本轮诊断首篇官方 API2 PDF 接口，响应明确为 `ChallengeRequiredError`；已登录
Chrome 与无凭据本地请求属于不同会话。浏览器自动保存/受保护接收的衔接尚未打通，
不将逐篇手动保存设为批量流程，亦不把浏览器导航中止误报为论文不存在。
...
| 离线验收 | 本轮 135 项：133 通过、2 个符号链接权限案例跳过；含四个实际强制中断后的新进程恢复 |
| 最小账本与保护 | 已有单写入账本、版本/租约、幂等接收、恢复和受管理 PDF 下载保护；不等于完整研究平台 |
| 真实研究 | 0 篇闭环；1 篇主轨正文、2 篇前作及 1 份补充材料已在本地；10 条便利候选，总体未知 |
