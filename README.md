...
G0-02 已完成真实浏览器 Pro 合成往返。2026-09-26 用户在本机交互窗口完成
官方 API 登录后，受保护程序自动取得首篇当前版（65 页）、原始投稿（49 页），
以及另一篇主轨论文（23 页）；第四次请求中断，分片保留，不计下载成功。
首篇 API 当前版与先前手动接收件逐字节相同。另取得 Lecué–Rigollet 2014
作者站点的 14 页前作。现有 **5 篇文献、7 个 PDF 对象，其中 2 篇为 ICML 2026
目标论文**；Mansour 2021 含正文/补充材料，Mourtada 2023 仅为两页扩展摘要。

首篇原始投稿来源已核实为官方附件；当前附件的 camera-ready 身份仍未认证。
两輪真实 Pro 返回（10 项初筛、6 项复核）及有限本地核对已保存；代码更新后旧包
保留历史 stale，不改旧哈希。新短任务已核验原文件和新增版本身份。新的完整文本
补充审读包包含首篇两个版本及三篇前作，共 146 页；实际发送/回传状态见
[P2 检查点](docs/P2_CHECKPOINT.md) 与 [脱敏状态](published/status.json)。
真实论文闭环仍为零，P2 未完成；人工审计、校准和实验复现未做。

最新代码完整验收 **150 项：148 通过、2 跳过、0 失败**，包括四项实际中断恢复。
独立 API 登录入口见[本地登录说明](docs/OPENREVIEW_API_LOGIN.md)。本机程序仅在
进程内处理用户输入，不保存凭据或读取浏览器会话；不再把逐篇手动保存作为批量方案。
下载器已降低重复目录计量开销，并记录具体的非敏感失败类别；保护限制仍生效。
第四份旧分片的确切中断原因未知，扫描开销导致超时只是待复测推测。

代码通过 Chrome 文本编辑器交付到功能分支；只有远端读回一致才记已同步。
正文、SQLite、完整抽取文本和原始交换包留本地。公开来源调查见
[来源核查](docs/PUBLIC_SOURCE_ROUTES.md)。未验证全会 PDF 库，也未作全会比例推断。
下文首批三文件交付描述属于历史。
...
| 离线验收 | 本轮 150 项：148 通过、2 个符号链接权限案例跳过；含四个实际强制中断后的新进程恢复 |
| 最小账本与保护 | 已有单写入账本、版本/租约、幂等接收、恢复和受管理 PDF 下载保护；不等于完整研究平台 |
| 真实研究 | 0 篇闭环；2 篇主轨论文、3 篇前作，共 7 个 PDF；10 条便利候选，总体未知 |
...
当前授权到 P2，一篇闭环后才扩展至最多十篇。G0 真实合成往返已完成；P2 已接收首篇正文并开展真实 Pro 初筛与独立复核，版本及前作覆盖仍有缺口，恢复时先读当前检查点。P3/P4 不自动启动。
