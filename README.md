...
G0-02 已完成真实浏览器 Pro 合成往返。新增受保护元数据入口后，完整离线验收
117 项（115 通过、2 跳过）。P2 已核实官方范围、保存十条候选题录并核对一篇
身份与公开录用决定；正文访问被拦，真实论文闭环仍为零。当前状态见
[P2 检查点](docs/P2_CHECKPOINT.md) 和 [脱敏状态](published/status.json)。
连接器仓库读取已恢复，写入返回 403；代码通过 Chrome 文本编辑器交付，
须以本地同步回执的远端读回验证为准。
...
| 离线验收 | 本轮 117 项：115 通过、2 个符号链接权限案例跳过；含四个实际强制中断后的新进程恢复 |
| 最小账本与保护 | 已有单写入账本、版本/租约、幂等接收、恢复和受管理 PDF 下载保护；不等于完整研究平台 |
| 真实研究 | 0 篇闭环、0 份正文；10 条便利候选、1 篇身份与公开录用决定已查；总体未知 |
