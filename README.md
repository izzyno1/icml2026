...
G0-02 本轮更新：最小交换协议已实现，完整离线验收 112 项（110 通过、2 跳过）。
真实 Pro 往返未做，真实论文闭环仍为零；等待 `G0_02_pro_001`。
连接器仓库读取本轮恢复，代码交付须以本地同步回执的远端读回验证为准。
工程范围与恢复方式见 [G0 检查点](docs/G0_EXCHANGE_CHECKPOINT.md) 和
[交换合约](contracts/pro_exchange.md)。下文首批三文件交付描述属于历史。

**本地负责可验证的执行，GPT Pro 对话负责有证据的分析与质疑，GitHub 保存经过审阅的代码、规则和结果版本。用户负责研究范围、重要裁决及发布决定。**

项目仓库：[izzyno1/icml2026](https://github.com/izzyno1/icml2026)，由用户指定，当前为公开仓库。首批文档交付范围仅为本文、`.gitignore`、`.github/PULL_REQUEST_TEMPLATE.md`。源码、规则文件、运行库及正文仍在本地，须在后续阶段分别审阅与同步。以下任务包、阶段发布和同步回执是实施方案，不能当作已经建好的自动同步系统。
...
| 离线验收 | 本轮 112 项：110 通过、2 个符号链接权限案例跳过；含四个实际强制中断后的新进程恢复 |
| 最小账本与保护 | 已有单写入账本、版本/租约、幂等接收、恢复和受管理 PDF 下载保护；不等于完整研究平台 |
| 真实研究 | 0 篇闭环、0 份论文正文；仅三条历史官方网页元数据来源，总体篇数未知 |
| Git | 项目分支 `codex/setup`；文档交付后衔接远端 main。代码尚未纳入首次基线 |
| Pro 任务包导出/回传导入 | 已实现并经合成离线测试；真实 Pro 返回仍待验收，结构通过不等于事实通过 |
| GitHub 同步 | 首批只含三份协作文件；每次远端读回结果记录在本地同步回执。连接器调用失败，Git 推送认证未测 |
...
普通 ChatGPT 项目不会因为写入了本地路径就获得电脑文件；需实际上传或使用已授权的数据连接。本文默认采用用户传递任务包，不依赖新插件。现有 GitHub 连接若确实可读，可以作为可选入口，但仍应核对文件版本，不能假定能写仓库、读到未提交文件或即时刷新到最新提交。[OpenAI 项目与聊天文档](https://learn.chatgpt.com/docs/projects)
...
任务包导出/隔离导入已实现；人工传递与真实 Pro 往返仍需实测，图表与论文发布链条未完成。
...
当前只准备 G0 协作与后续路线，不因写出计划就启动 P2 论文工作。正式新窗口先做 G0-02 和恢复检查，再做一篇至最多十篇。P3/P4 不自动启动。
...
下图中 `published/` 的论文结果子目录是计划，当前没有真实论文成果：
...
目标远端为 `https://github.com/izzyno1/icml2026.git`。本轮只交付三份协作文件；后续源码和规则基线须另行检查，不能把文档交付当作完整工程备份。
...
GitHub 写入连接或 Pro 回传暂缺时，本地可继续不依赖它的代码、测试和文件准备；不能伪造远端成功或 Pro 已读。本轮用户已授权 G0 通过后开始一篇至最多十篇；真实 Pro 门槛未过时不提前启动。
