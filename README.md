...
G0-02 已完成真实浏览器 Pro 合成往返。新增受保护镜像入口与预印本类型后，完整离线验收
124 项（122 通过、2 跳过）。P2 已核实官方范围、保存十条候选题录并核对一篇
身份与公开录用决定，并保存会议官网单篇元数据。用户确认 OpenReview 账户
待审核，已改查独立公开来源；已找到首篇的 alphaXiv、ResearchGate 70 页预印本链接，
网页服务可读，但本机分别因 robots 拒绝、HTTP 403 停止，正文未落盘，
真实论文闭环仍为零。当前状态见
...
| 离线验收 | 本轮 124 项：122 通过、2 个符号链接权限案例跳过；含四个实际强制中断后的新进程恢复 |
...
当前授权到 P2，一篇闭环后才扩展至最多十篇。G0 真实合成往返已完成；P2 已启动但正文访问受阻，恢复时先读当前检查点。P3/P4 不自动启动。
...
GitHub 写入连接或 Pro 回传暂缺时，本地继续独立的代码、测试和文件准备；不能伪造远端成功或 Pro 已读。本轮 G0 已通过，P2 在实际访问边界落盘，下一任务为 `P2_J4wRLmh29t_acquire_002`。先解决正文取证，不能用候选题录代替论文闭环。
