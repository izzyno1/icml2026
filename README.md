...
| 离线验收 | 91 项：89 通过，2 个符号链接权限案例跳过；含三种实际强制中断后的新进程恢复 |
...
| Pro 任务包导出/回传导入 | 待实现、待实测；现有模型结果不能直接当作可自动导入 |
...
这是目标流程；任务包导入导出程序尚未实现。初期人工传递文件仍需遵守相同版本检查。
...
这需要 G0-02 的实际代码实现，现有 `workflow.py` 尚无 `export-pro` / `import-pro` 命令。不要运行不存在的命令，也不要把一份 Pro JSON 直接写进 SQLite。
...
GitHub 写入连接或 Pro 回传暂缺时，本地可继续不依赖它的代码、测试和文件准备；不能伪造远端成功或 Pro 已读。当前用户尚未要求在本轮开始论文研究。
